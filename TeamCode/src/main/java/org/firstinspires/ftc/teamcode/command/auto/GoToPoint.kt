...
class GoToPoint(val target: Pose2D, val multiplier: Double = 1.0) : CommandTemplate() {
	constructor(x: Double, y: Double, theta: Double, multiplier: Double = 1.0) : this(
		Pose2D(x, y, theta),
		multiplier
	)
...
class GoToPointTimed(target: Pose2D, time: Long, multiplier: Double = 1.05) : ParallelRaceGroup(
	GoToPoint(target, multiplier),
	WaitCommand(time)
)