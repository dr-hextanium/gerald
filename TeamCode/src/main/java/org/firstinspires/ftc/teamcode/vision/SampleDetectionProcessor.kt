...
import org.firstinspires.ftc.teamcode.utility.geometry.Vec2d
...
class SampleDetector : VisionProcessor, CameraStreamSource {
    ...
    override fun processFrame(
        ...
        Imgproc.resize(
            baseFrame, frame, Size(baseFrame.width() * scale, baseFrame.height() * scale)
        )
        ...
        Imgproc.findContours(
            redThres, redContours, redHierarchy, Imgproc.RETR_EXTERNAL, Imgproc.CHAIN_APPROX_SIMPLE
        )
        Imgproc.findContours(
            yellowThres,
            yellowContours,
...
//        masked.setTo(Scalar.all(0.0))
        ...
        Imgproc.drawContours(
            masked, yellowData.map { it.contour }, -1, Scalar(255.0, 0.0, 255.0), 2
        )
        ...
        val contoursBitmap =
            Bitmap.createBitmap(masked.width(), masked.height(), Bitmap.Config.RGB_565)
    ...
    fun contoursToData(contours: List<MatOfPoint>): List<DetectionData> {
        return contours.map { contour ->
                ...
                val ratio =
                    min(rect.size.height / rect.size.width, rect.size.width / rect.size.height)
    ...
    fun dataToSamples(
        ...
        for (detection in data) {
            ...
            val center = Vec2d(1.0, -1.0) * (Vec2d(rect.center.x, rect.center.y) - Vec2d(
        ...
        return data.map { data ->
            val center = Vec2d(1.0, -1.0) * (Vec2d(data.rect.center.x, data.rect.center.y) - Vec2d(
                frame.width(),
                frame.height(),
            ) / 2)
            SampleDetection(
