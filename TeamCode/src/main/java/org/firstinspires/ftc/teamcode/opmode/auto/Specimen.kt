	...
	override fun start() {
		...
		Robot.scheduler.schedule(
			...
			SequentialCommandGroup(
				ParallelCommandGroup(
					// score the first one
					GoToPointTimed(Pose2D(36.0, 2.0, 0.0), 1600, 0.8),
					SequentialCommandGroup(
						WaitCommand(250),
						PrepScoringSpecimen(),
					)
				...
				GoToPointTimed(Pose2D(19.0, -24.0, 0.0), 2000),

				// go back
				GoToPointTimed(Pose2D(50.0, -30.0, 0.0), 1500, 0.8),
				// strafe to first
				GoToPointTimed(Pose2D(50.0, -40.0, 0.0), 1000, 2.0),

				// push in
				GoToPointTimed(Pose2D(18.0, -40.0, 0.0), 750, 1.2),
				// come back
				GoToPointTimed(Pose2D(50.0, -34.0, 0.0), 1000, 0.75),

				// strafe to second
				GoToPointTimed(Pose2D(50.0, -48.0, 0.0), 2000, 2.0),
				// push in
				GoToPointTimed(Pose2D(18.0, -50.0, 0.0), 750, 1.0),
				// come back
				GoToPointTimed(Pose2D(20.0, -50.0, 0.0), 1500, 1.1),
				// go up
				GoToPointTimed(Pose2D(2.0, -50.0, 0.0), 1000, 0.75),
				...
				PrepScoringSpecimen(),
				// inter 1
				GoToPointTimed(Pose2D(19.0, -10.0, 0.0), 1000),
				// inter 2
				GoToPointTimed(Pose2D(24.0, 0.0, 0.0), 1000),
				...
				// go to corner of 4 tiles
				GoToPointTimed(Pose2D(19.0, -30.0, 0.0), 2000),
				// push into third
				GoToPointTimed(Pose2D(3.0, -30.0, 0.0), 1000, 0.7),
				WaitCommand(250),
				...
				GoToPointTimed(Pose2D(24.0, 5.0, 0.0), 500, 0.8),
				GoToPointTimed(Pose2D(39.0, 5.0, 0.0), 2000, 0.8),
				ScoreSpecimen(),
			)
		)
	}
